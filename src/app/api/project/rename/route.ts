...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validation.valid) {
      return NextResponse.json({ error: validation.error, code: 'INVALID_NAME' }, { status: 400 })
    }
    ...
    if (!fs.existsSync(oldProjectPath)) {
      return NextResponse.json(
        {
          error: 'The project folder no longer exists at the expected location.',
          code: 'NOT_FOUND',
        },
        { status: 404 },
      )
    ...
    if (fs.existsSync(newClaudePath)) {
      return NextResponse.json(
        {
          error: 'A project with this name already exists in Claude data.',
          code: 'ALREADY_EXISTS',
        },
        { status: 409 },
      )
