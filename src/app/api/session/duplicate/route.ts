...
function stripToolResultsFromMessage(message: ChatMessage): ChatMessage {
  ...
  const filteredContent = message.message.content.filter((block) => block.type !== 'tool_result')
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (keepLastN !== undefined && (keepLastN <= 0 || !Number.isInteger(keepLastN))) {
      return NextResponse.json({ error: 'keepLastN must be a positive integer' }, { status: 400 })
    }
