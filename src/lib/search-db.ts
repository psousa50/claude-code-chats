...
export function createSearchDb(deps?: SearchDbDeps) {
  ...
  function removeFileFromIndex(filePath: string): void {
    ...
    if (fileRecord) {
      db.prepare('DELETE FROM messages_fts WHERE session_id = ? AND project_path = ?').run(
        fileRecord.session_id,
        fileRecord.project_path,
      )
  ...
  function renameProjectInIndex(oldEncodedPath: string, newEncodedPath: string): void {
    const transaction = db.transaction(() => {
      db.prepare('UPDATE indexed_files SET project_path = ? WHERE project_path = ?').run(
        newEncodedPath,
        oldEncodedPath,
      )

      db.prepare('UPDATE messages_fts SET project_path = ? WHERE project_path = ?').run(
        newEncodedPath,
        oldEncodedPath,
      )
...
export function syncIndex() {
  return getDefault().syncIndex()
}
export function search(query: string, limit?: number, projectPath?: string) {
  return getDefault().search(query, limit, projectPath)
}
export function getIndexStats() {
  return getDefault().getIndexStats()
}
export function getSummary(type: 'session' | 'project', targetId: string, projectPath: string) {
  return getDefault().getSummary(type, targetId, projectPath)
}
export function saveSummary(summary: Omit<AISummary, 'id'>) {
  return getDefault().saveSummary(summary)
}
export function getSessionSummaries(projectPath: string) {
  return getDefault().getSessionSummaries(projectPath)
}
export function getSessionSummariesFromDb(encodedPath: string) {
  return getDefault().getSessionSummariesFromDb(encodedPath)
}
export function getProjectStats() {
  return getDefault().getProjectStats()
}
export function renameProjectInIndex(oldEncodedPath: string, newEncodedPath: string) {
  return getDefault().renameProjectInIndex(oldEncodedPath, newEncodedPath)
}
