...
const noDbSessions = () => [] as { id: string; firstMessage: string; messageCount: number; lastActivity: number }[];
...
describe("getSessionsSummary", () => {
  ...
  it("falls back to filesystem when DB returns empty", () => {
    writeTempProject(projectsDir, "proj", {
      "s1": toJsonl([
        makeUserMessage("from filesystem"),
