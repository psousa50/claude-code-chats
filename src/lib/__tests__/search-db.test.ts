...
describe('syncIndex', () => {
  ...
  it('indexes a session file', () => {
    writeSession(
      'my-project',
      'sess1',
      toJsonl([makeUserMessage('hello world'), makeAssistantMessage('hi there')]),
    )
  ...
  it('does not index system messages', () => {
    writeSession(
      'my-project',
      'sess1',
      toJsonl([makeSystemMessage('system init'), makeUserMessage('real question')]),
    )
...
describe('search', () => {
  beforeEach(() => {
    ...
    writeSession('other-project', 'sess2', toJsonl([makeUserMessage('add dark mode to the UI')]))
    searchDb.syncIndex()
  })
...
describe('getIndexStats', () => {
  ...
  it('returns correct counts after sync', () => {
    writeSession('proj', 's1', toJsonl([makeUserMessage('msg1'), makeAssistantMessage('msg2')]))
    writeSession('proj', 's2', toJsonl([makeUserMessage('msg3')]))
    searchDb.syncIndex()
...
describe('getProjectStats', () => {
  it('returns stats grouped by project', () => {
    writeSession('proj-a', 's1', toJsonl([makeUserMessage('msg1'), makeAssistantMessage('msg2')]))
    writeSession('proj-b', 's1', toJsonl([makeUserMessage('msg3')]))
    searchDb.syncIndex()
