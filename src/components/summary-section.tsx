...
export function SummarySection({ type, projectPath, sessionId, messageCount }: SummarySectionProps) {
  ...
  useEffect(() => {
    async function fetchSummary() {
      try {
        ...
        // Ignore fetch errors - just means no cached summary
      } finally {
        setFetching(false);
      }
    }
    ...
    <div className="p-4 bg-neutral-900/50 border border-neutral-800 rounded-lg">
      <div className="flex items-center justify-between gap-4 mb-2">
        <h3 className="text-sm font-medium text-neutral-300 flex items-center gap-2">
          ...
          AI Summary
          {isStale && (
            <span className="text-xs text-amber-500/70">(outdated)</span>
          )}
        </h3>
        <button
          onClick={generateSummary}
          disabled={loading}
          className="px-3 py-1 text-xs font-medium text-amber-500 hover:text-amber-400 hover:bg-amber-500/10 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
