...
```ini
[Service]
Environment=CLAUDE_PATH=/home/yourname/.local/bin/claude
```

Alternatively, adding the directory containing `claude` to the service's `PATH` also works — though `CLAUDE_PATH` is preferred as it grants access to only the one binary needed.
